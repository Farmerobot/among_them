...
def get_phase_and_when_it_ends(history: List[History], players: List[Player], game_config: GameConfig) -> tuple[GamePhase, int]:
    """
    ...
    if history[-1].actions_until_phase_ends == 0:
        return handle_phase_change(history, players, previous_phase, game_config)
    
    return previous_phase, history[-1].actions_until_phase_ends - 1


def handle_phase_change(history: List[History], players: List[Player], previous_phase: GamePhase, game_config: GameConfig) -> tuple[GamePhase, int]:
    """
